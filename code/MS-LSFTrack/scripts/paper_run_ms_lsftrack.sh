...
if [ "$#" -lt 3 ]; then
  echo "Usage: bash scripts/paper_run_ms_lsftrack.sh DATASET_CONFIG CHECKPOINT TRACKER_NAME [SPLIT] [extra run_tracker args...]"
  echo
  echo "Example:"
  echo "  bash scripts/paper_run_ms_lsftrack.sh \\"
  echo "    configs/datasets/irdmstrack_v3_det_label.yaml \\"
  echo "    /path/to/checkpoints/ms_lsf_ird_v3/best.pt \\"
  echo "    ms_lsf_ird_v3_paper test"
  exit 2
fi
